...
function displayMessage(messageData, type = 'chat', prepend = false) {
    ...
    if (messageData.timestamp) { // Garder le timestamp sur l'item pour le scroll infini
        item.dataset.timestamp = messageData.timestamp;
    }

    let username = 'Anonymous';
    // let profilePic = 'assets/default-avatar.png'; // Tu peux l'utiliser si tu affiches l'avatar par message
    ...
    // ÉTAPE 1: Construire le contenu de 'item' en fonction du 'type'
    if (type === 'chat') {
        ...
        if (!isMyMessage && username && username !== 'Anonymous') {
            ...
            userStrong.classList.add('username');
            userStrong.textContent = escapeHtml(username);
            messageContentContainer.appendChild(userStrong);
            messageContentContainer.appendChild(document.createElement('br'));
        }

        if (messageData.content) {
            const textSpan = document.createElement('span');
            textSpan.classList.add('message-text');
            textSpan.textContent = escapeHtml(messageData.content);
            messageContentContainer.appendChild(textSpan);
        }
        ...
        if (messageData.fileAttachment) {
            ...
            fileLink.href = escapeHtml(messageData.fileAttachment.url);
            fileLink.target = '_blank';
            fileLink.classList.add('file-link');

            const iconElement = document.createElement('i');
            iconElement.className = 'fas fa-file';
            fileLink.appendChild(iconElement);

            const fileNameSpan = document.createElement('span');
            fileNameSpan.classList.add('file-name');
            fileNameSpan.textContent = ` ${escapeHtml(messageData.fileAttachment.name)}`;
            ...
            if (messageData.fileAttachment.size) {
                const fileSizeSpan = document.createElement('span');
                fileSizeSpan.classList.add('file-size');
                fileSizeSpan.textContent = ` (${formatFileSize(messageData.fileAttachment.size)})`;
                fileLink.appendChild(fileSizeSpan);
            }
            fileDiv.appendChild(fileLink);
            messageContentContainer.appendChild(fileDiv);
        }
        ...
        if (messageData.audioFile) {
            ...
            audioContainerDiv.classList.add('audio-message');

            const audioElement = document.createElement('audio');
            audioElement.controls = true;
            const sourceElement = document.createElement('source');
            sourceElement.src = escapeHtml(cleanAudioUrl);
            sourceElement.type = 'audio/webm';
            audioElement.appendChild(sourceElement);
            audioElement.appendChild(document.createTextNode('Your browser does not support the audio element.'));
            audioContainerDiv.appendChild(audioElement);

            // Empêcher le clic sur cette div audio de déclencher des événements sur les parents
            audioContainerDiv.addEventListener('click', function(event) {
                event.stopPropagation();
                console.log('Audio container clicked, propagation stopped.');
            });
            messageContentContainer.appendChild(audioContainerDiv);
        }
        ...
        const systemTextSpan = document.createElement('span');
        systemTextSpan.classList.add('system-text');
        systemTextSpan.textContent = escapeHtml(messageData.text || messageData.content || ''); // Gérer text ou content
        item.appendChild(systemTextSpan);
    ...
    } else { // Type de message non géré
        console.warn("displayMessage called with unhandled type:", type, messageData);
        return; // Ne pas ajouter l'élément si le type n'est pas géré
    }

    // ÉTAPE 2: Ajouter 'item' (maintenant construit) à la liste des messages
    if (prepend) {
...
// Assure-toi que bodyElement, themeToggleButton, chatWrapper, bgChoiceButtons sont définis
// avant ces fonctions ou passés en arguments.
...
// CORRIGE ICI : Les noms de classes doivent correspondre à ton CSS
const themes = ['theme-dark-green', 'theme-dark-blue', 'theme-saiyan'];
// Si tu as un thème par défaut/clair, ajoute-le aussi ici ET dans ton CSS
// const themes = ['default-theme', 'theme-dark-green', 'theme-dark-blue', 'theme-saiyan'];


// Adapte les icônes et labels pour refléter le thème vers lequel le bouton va switcher
const themeIcons = {
    'theme-dark-green': '🌀', // De dark-green vers dark-blue
    'theme-dark-blue': '🔥',  // De dark-blue vers saiyan
    'theme-saiyan': '☀️',     // De saiyan vers dark-green (ou ton premier/default thème)
    // 'default-theme': '🌙', // Si tu as un thème par défaut
};
const themeLabels = {
    'theme-dark-green': 'Dark Blue',
    'theme-dark-blue': 'Saiyan',
    'theme-saiyan': 'Dark Green', // ou 'Light'
    // 'default-theme': 'Dark Green',
};
let currentThemeIndex = 0; // Sera défini correctement par initializeTheme

function applyTheme(themeName) {
    console.log("===> applyTheme a été appelée avec :", themeName);

    if (!bodyElement) { console.error("bodyElement non trouvé!"); return; }
    if (!themes.includes(themeName)) {
        console.warn(`Thème "${themeName}" inconnu. Utilisation du premier thème: ${themes[0]}`);
        themeName = themes[0];
    }
    ...
    if (themeToggleButton) {
        ...
        const nextThemeClassForButton = themes[nextThemeIndex];

        if (themeIcons[nextThemeClassForButton]) {
            themeToggleButton.textContent = themeIcons[nextThemeClassForButton];
        } else {
             // Fallback si l'icône pour le prochain thème n'est pas définie
             // Tu peux aussi choisir de montrer l'icône du thème *actuel* ici
            themeToggleButton.textContent = themeIcons[themeName] || '🎨';
        }
        if (themeLabels[nextThemeClassForButton]) {
            themeToggleButton.setAttribute('aria-label', `Switch to ${themeLabels[nextThemeClassForButton]} mode`);
        } else {
            themeToggleButton.setAttribute('aria-label', `Switch to ${themeLabels[themeName]} mode` || 'Switch theme');
        }
    }
    localStorage.setItem('chatTheme', themeName);
    console.log(`UI Theme appliqué: ${themeName}`);
    console.log("===> Classe actuelle du body :", bodyElement.className);
}

function initializeTheme() {
    const savedTheme = localStorage.getItem('chatTheme');
    let initialTheme = themes[0]; // Par défaut le premier thème de la liste

    if (savedTheme && themes.includes(savedTheme)) {
        initialTheme = savedTheme;
    }
    applyTheme(initialTheme); // applyTheme mettra à jour currentThemeIndex et le bouton
}

const backgroundClasses = ['bg-default', 'bg-bg1', 'bg-bg2', 'bg-bg3', 'bg-bg4', 'bg-bg5', 'bg-bg6'];

function applyChatBackground(bgName) {
    console.log("===> applyChatBackground a été appelée avec :", bgName);
    if (!chatWrapper) { console.error("chatWrapper non trouvé!"); return; }
    ...
    if (backgroundClasses.includes(targetClass)) {
        ...
        console.log(`Chat BG appliqué: ${bgName}`);
        console.log("===> Classes actuelles de chatWrapper :", chatWrapper.className);
        if (bgChoiceButtons) {
            bgChoiceButtons.forEach(btn => btn.classList.toggle('active-bg', btn.dataset.bg === bgName));
        }
    } else {
        console.warn(`Classe de fond "${targetClass}" non trouvée dans backgroundClasses.`);
    }
}
...
// LES APPELS À initializeTheme() ET initializeChatBackground() SONT MIEUX PLACÉS DANS showChatView
// OU APRES QUE LES ÉLÉMENTS DU DOM SOIENT GARANTIS D'ÊTRE LÀ.

// ... (le reste de ton code, y compris les écouteurs d'événements pour themeToggleButton et bgChoiceButtons) ...

// Assure-toi que ces écouteurs sont bien placés dans DOMContentLoaded :
